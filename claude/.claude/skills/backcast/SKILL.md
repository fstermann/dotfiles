...
Offer 2 to 3 viable approaches with tradeoffs (effort, risk, reversibility, fit) and let the user choose. Don't default to the one they named. Put a real multi-axis comparison in prose or a table alongside the question; a simple pick can live in the option labels.
...
For small or already-scoped tasks the *written plan* can collapse to: goal, end-state, backward chain, first moves. That is output length only; this never licenses merging phases or skipping a confirmation.
